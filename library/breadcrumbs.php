...
if ( ! function_exists( 'ksasacademic_breadcrumb' ) ) {
	function ksasacademic_breadcrumb( $showhome = true, $separatorclass = false ) {
		...
		$blog_title = $theme_option['flagship_sub_feed_name'];
		...
		if ( ! is_front_page() ) {
			...
			if ( is_single() && ! is_attachment() ) {
				...
				if (is_singular('bulletinboard') ) {
					...
					echo '<li><a href="' . $blog_url . '">' . $blog_title . '</a></li>';
				}
				...
				// Paginated archives
				echo '<li class="current item-current-' . get_query_var('paged') . '">' . __('Archives Page', 'ksasacademic' ) . ' ' . get_query_var('paged') . '</li>';
				...
				$theme_option = flagship_sub_get_global_options();
				echo '<li class="current item-' . $post->ID . '">' . $theme_option['flagship_sub_feed_name'] . '</li>';
